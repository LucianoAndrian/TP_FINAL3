#--LUCIANO ANDRIAN TP FINAL LABORATORIO PARA EL PROCESAMIENTO DE INFORMACION METEOROLOGICA 
####################################################################\m/####################################################################
rm(list=ls())
library(maps)
library(readxl)
...
puntos <- data.frame(lon=estaciones[[1]], lat=estaciones[[2]])
mapa <- get_map(location = c(left = -75, bottom = -55, right=-50, top = -20 ), source = 'stamen', maptype = "toner-lite",zoom=6)
arg1 <- ggmap(mapa, extent = 'normal')+ggtitle("Estaciones")
arg<-arg1 + geom_point(data=puntos, col="red",size=1)

ggsave("ESTACIONES.jpg",plot = arg ,width = 10,height = 15 ,units = "cm")
...
colnames(pp_estacional)<-c(colnames(pp_arg[[12]]))  #necesario para seleccionar estaciones en el proximo paso
...
series<-array(0,c(33,5,6))

series[,5,]<-c(1980:2012) 

colnames(series)<-c("DEF","MAM","JJA","SON","ANIOS")

for(i in 1:6){
  series[,1:4,i]<-pp_estacional[,estaciones_seleccionadas[i],1:4]
}
...
for(i in 1:6){
  ...
  p1 <- ggplot(as.data.frame(series[,,i]), aes(x=ANIOS)) + geom_line(aes(x=ANIOS, y=DEF),col="firebrick") +
    xlab("Anios") + ylab("Precipitacion") + labs(title="DEF") + ylim(c(0,900))+ theme_bw()+theme(plot.background = element_rect(fill = colores[i]))
  p2 <- ggplot(as.data.frame(series[,,i]), aes(x=ANIOS)) + geom_line(aes(x=ANIOS, y=MAM),col="orange") +
    xlab("Anios") + ylab("Precipitacion") + labs(title="MAM") + ylim(c(0,900))+ theme_bw() +theme(plot.background = element_rect(fill = colores[i]))
  p3 <- ggplot(as.data.frame(series[,,i]), aes(x=ANIOS)) + geom_line(aes(x=ANIOS, y=JJA),col="deepskyblue") +
    xlab("Anios") + ylab("Precipitacion") + labs(title="JJA") + ylim(c(0,900))+ theme_bw()+theme(plot.background = element_rect(fill = colores[i]))
  p4 <- ggplot(as.data.frame(series[,,i]), aes(x=ANIOS)) + geom_line(aes(x=ANIOS, y=SON),col="forestgreen") +
    xlab("Anios") + ylab("Precipitacion") + labs(title="SON") + ylim(c(0,900))+ theme_bw()+theme(plot.background = element_rect(fill = colores[i]))
  Q[[i]] <- grid.arrange(p1,p2,p3,p4, nrow=2,ncol=2, top=textGrob(names(series)[i], gp=gpar(fontsize=15,font=1)))
  
}

ggsave("serie_1.png",plot = grid.arrange(Q[[1]],Q[[2]],ncol=2,top="Series PP acumulada estacional"),width = 30,height = 15 ,units = "cm")
ggsave("serie_2.png",plot = grid.arrange(Q[[3]],Q[[4]],ncol=2,top="Series PP acumulada estacional"),width = 30,height = 15 ,units = "cm")
ggsave("serie_3.png",plot = grid.arrange(Q[[5]],Q[[6]],ncol=2,top="Series PP acumulada estacional"),width = 30,height = 15 ,units = "cm")

####################################################################\m/####################################################################

#LOS DATOS ESTAN COMPLETADOS GENERANDO TENDENCIAS NEGATIVAS EN REGIONES DONDE LA TENDENCIA OBSERVADA ES POSITIVA
...
pp_tend<-ggmap(mapa, extent = 'normal') +
  ...
  guides(color=guide_legend(), size = guide_legend()) + ggtitle("TENDENCIAS PP 1979-2012")

ggsave("Tendencia_PP.jpg",plot = pp_tend ,width = 10,height = 13 ,units = "cm")
...
indices_promedio<-array(NA,dim=c(34,4,6)) #2DA DIMENCION CADA ESTACION DEL ANIO,3ERA CADA INDICE
...
for(i in 1:24)
  ...
  V[[i]]<<-ggplot() +
    geom_map(data = mymap, map = mymap, aes(x = long, y = lat, map_id = id), fill = "white", color = "black",lwd=1.2) +
    geom_tile(data = dInterp, aes(x = x, y = y, fill = z), alpha = 0.8) + 
...
ggsave("Corr_AAO_PP.JPG",plot=grid.arrange_tp.final(V[[1]],V[[2]],V[[3]],V[[4]],ncol=2,nrow = 2,position = "right",titulo = "CORRELACION AAO VS PP ACUMULADA"),width = 15,height = 15 ,units = "cm")
ggsave("Corr_DOI_pp.JPG",plot=grid.arrange_tp.final(V[[5]],V[[6]],V[[7]],V[[8]],ncol=2,nrow = 2,position = "right",titulo = "CORRELACION DOI VS PP ACUMULADA"),width = 15,height = 15 ,units = "cm")
ggsave("Corr_NINO3.4_PP.JPG",plot=grid.arrange_tp.final(V[[9]],V[[10]],V[[11]],V[[12]],ncol=2,nrow = 2,position = "right",titulo = "CORRELACION NINO3.4 VS PP ACUMULADA"),width = 15,height = 15 ,units = "cm")
ggsave("Corr_SOI_PP.JPG",plot=grid.arrange_tp.final(V[[13]],V[[14]],V[[15]],V[[16]],ncol=2,nrow = 2,position = "right",titulo = "CORRELACION SOI VS PP ACUMULADA "),width = 15,height = 15 ,units = "cm")
ggsave("Corr_SAODI_pp.JPG",plot=grid.arrange_tp.final(V[[17]],V[[18]],V[[19]],V[[20]],ncol=2,nrow = 2,position = "right",titulo = "CORRELACION SAODI VS PP ACUMULADA"),width = 15,height = 15 ,units = "cm")
ggsave("Corr_OLR_PP.JPG",plot=grid.arrange_tp.final(V[[21]],V[[22]],V[[23]],V[[24]],ncol=2,nrow = 2,position = "right",titulo = "CORRELACION OLR VS PP ACUMULADA"),width = 15,height = 15 ,units = "cm")
